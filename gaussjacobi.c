...
int main(){
    ...
    do{
        if(i > 0 && i % 100 == 0){
            ...
            printf("\n[Warning] However it is also possible that the given system of equations is solvable, but is not diagonally dominant, which in turn renders it insolvable by Gauss-Jacobi's method.");
        ...
        if(i == 1 || neq(table[i-2][1], table[i-1][1])){
            table[i][1] = fy(table[i][0], table[i-1][2]);
        ...
        if(i == 1 || neq(table[i-2][2], table[i-1][2])){
            table[i][2] = fz(table[i][0], table[i][1]);
